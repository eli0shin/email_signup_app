...
function escape(field) {
 ...
 /*
  ...
  {fields} - json fields to convert to csv
  {fieldName} - field names to use
  {quotes} - quotation marks to use in csv
  {csvHeaders} - If true uses the keys of the objects in {obj} to set a header row
  {headers} - Optional HTTP response headers
  {status} - Optional status code
 */

res.csv = function(data, fields, fieldNames, quotes, csvHeaders, headers, status) {
  var body = '';
  var headerRow = [];
  ...
  /*for (var i = 0; i < data.length; i++) {
    ...
    body += json2csv({ data: data, fields: fields, fieldNames: ['Email Address'], quotes: quotes });
    //data[i].map(escape).join(exports.separator) + '\r\n';
